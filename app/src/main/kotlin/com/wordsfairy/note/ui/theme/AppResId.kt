package com.wordsfairy.note.ui.theme

import com.wordsfairy.note.R

/**
 * @Description:
 * @Author: JIULANG
 * @Data: 2023/4/29 14:10
 */

object AppResId {

    object Drawable {
        const val Set =  R.drawable.app_ic_set
        const val Folder =  R.drawable.app_ic_folder
        const val Forward =  R.drawable.app_ic_forward
        const val Copy =  R.drawable.app_ic_copy
        const val Delete =  R.drawable.app_ic_delete
        const val Search =  R.drawable.app_ic_search
        const val ReadMode =  R.drawable.app_ic_read_mode
        const val Modify =  R.drawable.app_ic_modify
        const val Arrow_Right =  R.drawable.app_ic_arrow_right
        const val Correct =  R.drawable.app_ic_correct
    }
    object String {
        //新建文件夹
        const val CreateFolder =  R.string.app_create_folder
        const val Cancel =  R.string.app_cancel
        const val Confirm =  R.string.app_confirm
        const val FolderName =  R.string.app_folder_name
        const val NoteContentText =  R.string.app_note_content_text
        const val Search =  R.string.app_search
        const val Set =  R.string.app_set
    }
}